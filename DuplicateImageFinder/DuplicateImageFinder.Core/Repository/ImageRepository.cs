    ...
    public class ImageRepository : IFileRepository
    {
        ...
        public List<FileInfoWithHash> GetFiles(string path)
        {
            if (string.IsNullOrEmpty(path))
                throw new ArgumentException("Image Repository failed: Path can not be null of empty");

            if (!Directory.Exists(path))
                throw new DirectoryNotFoundException("Image Repository failed: Directory provided does not exist");
